...
static save_button_t *save_new_button(char *txt, sfVector2f pos)
{
    ...
    new->text = create_text(new->font, txt, 80, (sfVector2f){pos.x, pos.y + 60});
    new->new_txt = create_text(new->font, "NOUVELLE PARTIE", 60, (sfVector2f){
        pos.x, pos.y + 500});
    new->state = NOTHING;
    ...
    save_button_t *new, char *str, int size, sfVector2f pos)
{
    ...
    sfText_setPosition(text, (sfVector2f){pos.x + 120, pos.y + 510 + i});
    i += 130;
    return text;
}
...
save_menu_t *init_save_menu(rpg_t *rpg)
{
    ...
    menu->font = sfFont_createFromFile("assets/fonts/PoetsenOne-Regular.ttf");
    menu->text = NULL;
