...
{
    tmp2->type = tmp->type;
    tmp2->armor_type = tmp->armor_type;
    tmp2->item = tmp->item;
    tmp->name = tmp2->name;
    tmp2->name = NULL;
}
...
static void slot_click(slot_t *tmp, slot_t *tmp2, rpg_t *rpg)
{
    ...
    for (; tmp2; tmp2 = tmp2->next) {
        ...
        (tmp2->access == ALL || (tmp->type == WEAPON && tmp2->access == WEAPON) ||
        (tmp->type == ARMOR && tmp2->access == ARMOR && tmp->armor_type == tmp2->access_armor))) {
            tmp2->armor_type = (tmp->type == ARMOR) ? ((armor_t *)tmp->item)->armor_type : 0;
            tmp2->is_empty = 0;
            tmp2->is_active = 0;
