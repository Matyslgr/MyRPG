...
skill_t *init_all_skill(void)
{
    ...
    skill = add_skill(skill,
    init_skill("Poison sword", "Shoot a poison", 10, &skill_poison_sword));
    setup_logo(skill->next->next->next->next,
    "assets/skill/poison.png");
    skill = add_skill(skill,
    init_skill("Fire sword", "Shoot fire", 10, &skill_fire_sword));
...
static int click_skill(rpg_t *rpg, skill_t *tmp)
{
    ...
    rpg->event.mouseButton.button == sfMouseLeft)
        unlock_skill(rpg, rpg->skill_tree, tmp->name);
    return 0;
}
